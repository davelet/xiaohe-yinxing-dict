...
- **完整帮助文档** — 内置小鹤音形完整帮助文档
- **外部 Rime 词典管理** — 加载、搜索本地 Rime 格式词典文件

## 外部 Rime 词典管理

应用内置了一个独立的"输入法数据"视图（`ViewMode::Manager`），用于加载和查询用户本地的 Rime 格式词典文件（`.dict.yaml` / `.txt`）。

功能包括：自动扫描 `~/Library/Rime/` 目录发现词典文件、手动添加文件路径、逐个启用/禁用、实时搜索（复用同一套泛型搜索引擎）、一键复制，以及配置持久化。

**与主视图的关系：** 外部词典的数据（`ExternalDictEntry`）、搜索引擎（`SearchEngine<ExternalDictEntry>`）、UI 状态（`ManagerState`）和配置（`AppConfig`）完全独立于主视图（`ViewMode::Dict`）。两套数据不共享、不混合，各自维护独立的生命周期。唯一的依赖是入口——Manager 视图通过顶层 `ViewMode` 枚举切换进入，与主视图共用同一个应用窗口和标题栏导航。

## 帮助文档

本项目包含完整的小鹤音形帮助文档，由 `src/help/mod.rs` + `src/help/renderers.rs` 使用静态 Rust 代码直接渲染（egui 原生 widget），无需联网、无需外部模板文件。
...
帮助文档渲染使用**普通函数**（而非 `macro_rules!`）实现，理由如下：

- **函数指针驱动分发** — `HelpChapter.render` 的类型为 `fn(&mut Ui, &mut HelpNav, &HelpManager)`，宏无法作为函数指针传递，若改用宏则需重写整个章节分发机制。
- **调用处已足够简洁** — `h1(ui, "标题")` 仅比 `h1!("标题")` 多 `ui,` 三个字符，收益极低。
- **辅助函数体极小**（1–4 行），宏无法缩减有意义的工作量。
- **`lnk`/`navrow` 同时依赖 `ui` 和 `nav`** — `macro_rules!` 不具备真正的变量捕获能力，要让宏自动获取这两个局部变量需设计复杂的模式，反而降低可读性。
- **IDE 支持完备** — 函数能获得完整的跳转、重命名、类型检查支持，`macro_rules!` 在这些方面差很多。

所有辅助函数定义在 `src/help/mod.rs:196–374`，每个章节渲染器定义在 `src/help/renderers.rs` 中。
