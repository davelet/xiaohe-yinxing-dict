...
3. **工具集成** - search_text（汉字查编码）、search_code（编码查汉字）、get_help（帮助文档）、list_categories（分类列表）、get_category_stats（分类统计）、search_external_dict（外部词典搜索）
4. **设置表单** - 提供商选择、API Key 管理（含清除）、模型配置、Function Calling 开关
...
10. **快捷操作按钮** - 复制编码、在词典中查看（跳转词典视图）、复制回复、重新生成
11. **系统代理状态显示** - 设置页显示检测到的系统代理
...
7. ~~**假流式输出**~~ ✅ 已修复 - 移除逐字 sleep 模拟，改为真正的流式输出（`src/ai/client.rs`）
...
11. ~~**无快捷操作按钮**~~ ✅ 已修复 - 添加复制编码、词典中查看、复制回复、重新生成按钮（`src/ui/chat_viewport.rs`）
12. ~~**缺少 3 个工具**~~ ✅ 已修复 - 实现 get_help、list_categories、get_category_stats、search_external_dict（`src/ai/tools.rs`）
...
### 已完成新增功能（本轮）：
17. ✅ **search_external_dict 工具** - 实现外部 Rime 词典搜索（`src/ai/tools.rs`）
18. ✅ **重新生成按钮** - 点击后移除最后一条 AI 消息并重发最后一条用户消息（`src/ai/chat.rs` + `src/ui/chat_viewport.rs`）
19. ✅ **复制回复按钮** - 复制 AI 完整回复内容（`src/ui/chat_viewport.rs`）
20. ✅ **工具降级** - Function Calling 关闭时将工具描述注入 system prompt，模型以文本格式调用（`src/ai/client.rs` + `src/ai/config.rs`）
21. ✅ **日志调试** - 关键事件输出到 stderr（`src/ai/mod.rs` + `src/ui/chat_viewport.rs`）
22. ✅ **系统代理状态显示** - 设置页显示检测到的系统代理（`src/ui/chat_viewport.rs`）

### 遗留问题（已评估，暂不实现或已知行为）：
1. **三栏布局** - ❌ 不合理：子窗口较小，三栏会拥挤；建议保持单栏，对话历史用下拉/弹窗
2. **主窗口垂直居中边界** ⚠️ 待确认 - 首次定位用 `monitor_size.y - size.y` 计算居中位置，若窗口高度超过显示器高度会出现负坐标（`src/ui/mod.rs`）
3. **多显示器首次定位重试** ⚠️ 已知行为 - 首帧 `viewport().outer_rect` 可能尚未建立（返回 None），`main_window_positioned` 会保持 false 并每帧重试发命令，直到拿到尺寸后才置位；属预期重试逻辑，但需确认无遗漏（`src/ui/mod.rs`）